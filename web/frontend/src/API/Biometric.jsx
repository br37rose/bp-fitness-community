...
import {
    BP8_FITNESS_BIOMETRICS_MY_SUMMARY_API_ENDPOINT,
    BP8_FITNESS_BIOMETRICS_HISTORIC_DATA_API_ENDPOINT,
...
export function getMySummaryAPI(filtersMap=new Map(), onSuccessCallback, onErrorCallback, onDoneCallback) {
    const axios = getCustomAxios();

    // The following code will generate the query parameters for the url based on the map.
    let aURL = BP8_FITNESS_BIOMETRICS_MY_SUMMARY_API_ENDPOINT;
    filtersMap.forEach(
        (value, key) => {
            let decamelizedkey = decamelize(key)
            if (aURL.indexOf('?') > -1) {
...
export function getHistoricDataAPI(filtersMap=new Map(), onSuccessCallback, onErrorCallback, onDoneCallback) {
    const axios = getCustomAxios();

    // The following code will generate the query parameters for the url based on the map.
    let aURL = BP8_FITNESS_BIOMETRICS_HISTORIC_DATA_API_ENDPOINT;
    filtersMap.forEach(
        (value, key) => {
            let decamelizedkey = decamelize(key)
            if (aURL.indexOf('?') > -1) {

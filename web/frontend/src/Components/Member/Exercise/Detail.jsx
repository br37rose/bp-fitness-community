  ...
  return (
    <>
      {showVideo ? (
          ...
          {(() => {
            switch (videoType) {
              ...
              case EXERCISE_VIDEO_TYPE_VIMEO:
                return <Vimeo video={`${videoUrl}`} autoplay />;
              default:
                return null;
            }
          })()}
        ...
        >
          {thumbnailType === EXERCISE_THUMBNAIL_TYPE_SIMPLE_STORAGE_SERVER && (
            <img
              className="border-radius image is-256x256"
              src={thumbnailObjectUrl}
              alt={`Thumbnail`}
              style={{ cursor: "pointer" }}
            />
          )}
          {thumbnailType === EXERCISE_THUMBNAIL_TYPE_EXTERNAL_URL && (
            <img
              className="border-radius image is-256x256"
              src={thumbnailUrl}
              alt={`Thumbnail`}
...
const VideoDescription = ({
  ...
  <div>
    <h4 class="is-size-6 has-text-grey-light has-text-weight-bold mb-1">
      DESCRIPTION
    </h4>
    <p>{description}</p>
    <h4 class="is-size-6 has-text-grey-light has-text-weight-bold mb-1 mt-4">
      CATEGORY
    </h4>
    <p>
    ...
    <p>
      {
        EXERCISE_MOMENT_TYPE_OPTIONS.find((item) => item.value === movementType)
          ?.label
      }
    </p>
    <h4 class="is-size-6 has-text-grey-light has-text-weight-bold mb-1 mt-4">

...
*/
function MemberVideoContentListMobile(props) {
        ...
        <>
            {listData && listData.results && listData.results.map(function (datum, i) {
                return <div class="pb-2" key={`mobile_tablet_${datum.id}`}>
                    ...
                    <br />
                    <br />
                    <strong>Type:</strong>&nbsp;{VIDEO_CONTENT_VIDEO_TYPE_MAP[datum.type]}
                    <br />
                    <br />
                    <strong>Offer:</strong>&nbsp;
                    {datum.offerName
                    ...
                    <div class="is-hidden-mobile" key={`tablet_${datum.id}`}>
                        <div className="buttons is-right">
                            <Link to={`/video-collection/${datum.collectionId}/video-content/${datum.id}`} class="button is-small is-dark" type="button">
                                <FontAwesomeIcon className="mdi" icon={faEye} />&nbsp;View
                            </Link>
                            {/*
                            <Link to={`/member/${datum.id}/update`} class="button is-small is-warning" type="button">
                    ...
                    <div class="is-hidden-tablet" key={`mobile_${datum.id}`}>
                        <div class="columns is-mobile">
                            <div class="column">
                                <Link to={`/video-collection/${datum.collectionId}/video-content/${datum.id}`} class="button is-small is-dark is-fullwidth" type="button">
                                    <FontAwesomeIcon className="mdi" icon={faEye} />&nbsp;View
                                </Link>
                            </div>
                            {/*
                            <div class="column">
                                <Link to={`/member/${datum.id}/update`} class="button is-small is-warning is-fullwidth" type="button">
                                    <FontAwesomeIcon className="mdi" icon={faPencil} />&nbsp;Edit
                                </Link>
                            </div>
                            <div class="column">
...
export default MemberVideoContentListMobile;

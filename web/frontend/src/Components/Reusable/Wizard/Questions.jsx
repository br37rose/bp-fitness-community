...
import { faArrowLeftLong, faArrowRight, faArrowRightLong, faCheck } from "@fortawesome/free-solid-svg-icons";
import {
    faBars,
...
const Title = ({ text, subtitle, quizTime }) => (
    <div>
        ...
        <h2 className="subtitle is-size-3 has-text-centered has-text-white">{subtitle}</h2>
        <p className="is-size-7 has-text-centered has-text-white pb-3">{quizTime}</p>
    </div>
);
        ...
        >
            <div className="card-image">
                <figure className="image is-4by3">
                    <img src={imgUrl} alt="User" style={{ objectFit: 'cover', height: '100%' }} />
                </figure>
            </div>
            <div className="button is-info is-fullwidth">
                {buttonText}:&nbsp;{card.type === 'age' ? card.ageRange : ''}&nbsp;<span className="icon">&nbsp;<FontAwesomeIcon icon={faArrowRight} /></span>
            </div>
        </div>
    </div>
);
...
const SelectableOption = ({ option, isSelected, onSelect }) => (
    <div className="field">
        <button
            ...
            className={`button is-large is-fullwidth ${isSelected ? 'is-primary' : 'is-dark'}`}
        >
            <span>{option}</span>
            {isSelected && (

...
function AccountMoreOperationSubscribe() {
	...
	function onCreateStripeSubscriptionCheckoutSessionSuccess(response) {
		console.log(
			"onCreateStripeSubscriptionCheckoutSessionSuccess: Starting..."
		);
		console.log(
			"onCreateStripeSubscriptionCheckoutSessionSuccess: checkoutUrl:",
					...
					<>
						<div>
									...
									listData.results.map((content, index) => (
										<div className="column is-half is-flex">
											<PricingTable
												...
												title={content.name}
												highlightedText={
													content.recommended ? "Recommended" : null
												}
												customClasses={
													content.recommended ? "highlighted-border" : ""
												}
												price={content.price}
												period={null}
							...
							<div className="columns pt-5">
								<Link class="button is-hidden-touch" to={`/account`}>
...
const PricingTable = (props) => <PricingOption {...props} />;
...
const PricingOption = ({
	...
	<div
		class={`card is-flex is-flex-direction-column ${customClasses} ${
			isLocked ? "is-locked" : ""
		}`}
		style={{...customStyles, minWidth: "100%", position: "relative"}}>
		{isLocked && (
		...
		{highlightedText && (
			<div
				...
				class="highlightedText-badge is-uppercase has-text-centered has-background-primary has-text-white has-text-weight-semibold py-2">
				{highlightedText}
			</div>
		)}
		...
		<div class="card-content is-flex is-flex-direction-column is-flex-grow-1">
			{title && (
				<h3 class="title is-5 has-text-centered is-primary mb-4">{title}</h3>
			)}
			{subtitle && (
				<p class="subtitle has-text-centered is-6 mb-5">{subtitle}</p>
			)}
			<Pricing price={price} period={period} />
			<div class="is-flex-grow-1">
				{description && (
					<p class="subtitle has-text-centered is-6 my-5">{description}</p>
				)}

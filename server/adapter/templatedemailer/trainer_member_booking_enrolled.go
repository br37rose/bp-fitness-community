...
func (impl *templatedEmailer) SendMemberBookedSessionEmailToTrainer(trainerEmail string, trainerFirstName string, workoutProgramName string, branchID string, memberID string, memberName string, startAt time.Time) error {
	fp := path.Join("templates", "trainer_member_booking_enrolled.html")
	...
	if err := impl.Emailer.Send(context.Background(), impl.Emailer.GetSenderEmail(), "You are booked", trainerEmail, body); err != nil {

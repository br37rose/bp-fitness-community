...
func (impl *templatedEmailer) SendMemberCancelledBookingEmailToMember(memberEmail string, memberFirstName string, bookingID string, workoutProgramName string, trainerName string, startAt time.Time) error {
	////
	//// Send notification email to member.
	////

	fp := path.Join("templates", "member_booking_cancelled.html")
	...
	data := struct {
		...
		DetailsLink:        "https://" + impl.Emailer.GetFrontendDomainName() + "/booking/" + bookingID,
	...
	if err := impl.Emailer.Send(context.Background(), impl.Emailer.GetSenderEmail(), "You canceled a booking", memberEmail, body); err != nil {

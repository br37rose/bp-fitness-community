...
func (c *VideoContentControllerImpl) ListByFilter(ctx context.Context, f *vcon_s.VideoContentListFilter) (*vcon_s.VideoContentListResult, error) {
	...
	listRes, err := c.VideoContentStorer.ListByFilter(ctx, f)
	if err != nil {
	...
	// Iterate through all the videocontents and refresh the presigned URL if it has expired.
	for _, e := range listRes.Results {

		// Refresh the presigned URL if it has expired or else skip this step.
		if e.VideoType == vcon_s.VideoContentVideoTypeS3 && e.VideoObjectKey != "" {
			if time.Now().After(e.VideoObjectExpiry) {
				c.Kmutex.Lockf("videocontent-%v", e.ID.Hex()) // Step 1
				defer func() {
					c.Kmutex.Unlockf("videocontent-%v", e.ID.Hex()) // Step 2
				}()
				...
				if err := c.VideoContentStorer.UpdateByID(ctx, e); err != nil {
		...
		// Refresh the presigned URL if it has expired or else skip this step.
		if e.ThumbnailType == vcon_s.VideoContentThumbnailTypeS3 && e.ThumbnailObjectKey != "" {

...
func (impl *AggregatePointControllerImpl) AggregateThisMonthForAllActiveGoogleFitApps(ctx context.Context) error {
	...
	for _, gfaID := range res {
		...
		// impl.Logger.Debug("aggregate this month",
		// 	slog.Any("start", start),
		// 	slog.Any("end", end))

		if err := impl.aggregateForMetric(ctx, gfa.HeartRateBPMMetricID, ap_s.PeriodMonth, start, end); err != nil {

...
type RankPointPaginationListFilter struct {
	// Pagination related.
	Cursor    string
	PageSize  int64
	SortField string
...
type RankPointPaginationListResult struct {
...
func (impl RankPointStorerImpl) newPaginationFilter(f *RankPointPaginationListFilter) (bson.M, error) {
	if len(f.Cursor) > 0 {
...
func (impl RankPointStorerImpl) newPaginationFilterBasedOnString(f *RankPointPaginationListFilter, decodedCursor string) (bson.M, error) {
...
func (impl RankPointStorerImpl) newPaginationFilterBasedOnStart(f *RankPointPaginationListFilter, decodedCursor string) (bson.M, error) {
	// Extract our cursor into two parts which we need to use.
	arr := strings.Split(decodedCursor, "|")
	if len(arr) < 1 {
		return nil, fmt.Errorf("cursor is corrupted for the value `%v`", decodedCursor)
...
func (impl RankPointStorerImpl) newPaginationOptions(f *RankPointPaginationListFilter) (*options.FindOptions, error) {
...
// with our paginator.
func (impl RankPointStorerImpl) newPaginatorNextCursor(f *RankPointPaginationListFilter, results []*RankPoint) (string, error) {
	var lastDatum *RankPoint
	...
	case "metric_id":
		...
		break
	case "start":
		timestamp := lastDatum.Start.UnixMilli()

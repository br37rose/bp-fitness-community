...
func (impl FitnessChallengeStorerImpl) ListByFilter(
	ctx context.Context,
	f *FitnessChallengeListFilter) (*FitnessChallengeListResult, error) {
	...
	results := []*FitnessChallenge{}
	hasNextPage := false
	for cursor.Next(ctx) {
		var tp FitnessChallenge
	...
	return &FitnessChallengeListResult{
		Results:     results,

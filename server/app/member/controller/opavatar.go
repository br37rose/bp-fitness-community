...
type MemberOperationAvatarRequest struct {
...
func (impl *MemberControllerImpl) validateOperationAvatarRequest(ctx context.Context, dirtyData *MemberOperationAvatarRequest) error {
	e := make(map[string]string)

	if dirtyData.MemberID.IsZero() {
		e["member_id"] = "missing value"
	}
...
func (impl *MemberControllerImpl) Avatar(ctx context.Context, req *MemberOperationAvatarRequest) (*u_s.User, error) {
	...
	transactionFunc := func(sessCtx mongo.SessionContext) (interface{}, error) {
		...
		s, err := impl.UserStorer.GetByID(sessCtx, req.MemberID)
		if err != nil {
		...
		if req.File != nil {
			...
			objectKey := fmt.Sprintf("org/%v/uploads/user/%v/avatar/%v", orgID.Hex(), req.MemberID.Hex(), req.FileName)

...
func (impl *BiometricControllerImpl) generateSummaryForHR(ctx context.Context, u *u_d.User, res *AggregatePointSummaryResponse) error {
	// --- This Hour --- //
	thisHourStart, thisHourEnd := timekit.HourRangeForNow(time.Now)
	thisHour, err := impl.AggregatePointStorer.GetByCompositeKey(ctx, u.PrimaryHealthTrackingDeviceHeartRateMetricID, ap_s.PeriodHour, thisHourStart, thisHourEnd)
	if err != nil {
	...
	lastHour, err := impl.AggregatePointStorer.GetByCompositeKey(ctx, u.PrimaryHealthTrackingDeviceHeartRateMetricID, ap_s.PeriodHour, lastHourStart, lastHourEnd)
	if err != nil {
		impl.Logger.Error("failed getting aggregate point by composite key",
			slog.String("metric_id", u.PrimaryHealthTrackingDeviceHeartRateMetricID.Hex()),

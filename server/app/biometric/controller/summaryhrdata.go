...
func (impl *BiometricControllerImpl) generateSummaryDataForHR(sessCtx mongo.SessionContext, u *u_d.User, res *AggregatePointSummaryResponse, mu *sync.Mutex, wg *sync.WaitGroup) error {
	...
	thisDayFilter := &ap_s.AggregatePointPaginationListFilter{
		...
		MetricIDs: []primitive.ObjectID{u.PrimaryHealthTrackingDevice.HeartRateBPMMetricID},
	...
	if lastYearList != nil {
		...
		// 	slog.Time("start", lastYearStart),
		// 	slog.Time("end", lastYearEnd))
		res.HeartRateLastYearData = lastYearList.Results
	}

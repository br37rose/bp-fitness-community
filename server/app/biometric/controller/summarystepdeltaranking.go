...
func (impl *BiometricControllerImpl) generateSummaryRankingsForStepsDelta(sessCtx mongo.SessionContext, u *u_d.User, res *AggregatePointSummaryResponse, mu *sync.Mutex, wg *sync.WaitGroup) error {
	...
	// --- Today --- //
	go func() {
		if err := impl.generateSummaryRankingsForStepsDeltaToday(sessCtx, u, res, mu, wg); err != nil {
			// return err
		}
	}()

	go func() {
		if err := impl.generateSummaryRankingsForStepsDeltaISOWeek(sessCtx, u, res, mu, wg); err != nil {
			//return err
		}
	}()

	go func() {
		if err := impl.generateSummaryRankingsForStepsDeltaMonth(sessCtx, u, res, mu, wg); err != nil {
			// return err
		}
	}()

	go func() {
		if err := impl.generateSummaryRankingsForStepsDeltaYear(sessCtx, u, res, mu, wg); err != nil {
			// return err
		}
	}()
	return nil
...
func (impl *BiometricControllerImpl) generateSummaryRankingsForStepsDeltaToday(sessCtx mongo.SessionContext, u *u_d.User, res *AggregatePointSummaryResponse, mu *sync.Mutex, wg *sync.WaitGroup) error {
	// Once this function has been completed (whether successfully or not) then
	// update the `WaitGroup` that this goroutine is finished.
	defer wg.Done()
	...
	rp, err := impl.RankPointStorer.GetByCompositeKeyForToday(sessCtx, u.PrimaryHealthTrackingDevice.StepCountDeltaMetricID, rp_d.FunctionAverage)
	if err != nil {
...
func (impl *BiometricControllerImpl) generateSummaryRankingsForStepsDeltaISOWeek(sessCtx mongo.SessionContext, u *u_d.User, res *AggregatePointSummaryResponse, mu *sync.Mutex, wg *sync.WaitGroup) error {
	// Once this function has been completed (whether successfully or not) then
	// update the `WaitGroup` that this goroutine is finished.
	defer wg.Done()
	...
	rp, err := impl.RankPointStorer.GetByCompositeKeyForThisISOWeek(sessCtx, u.PrimaryHealthTrackingDevice.StepCountDeltaMetricID, rp_d.FunctionAverage)
	if err != nil {
...
func (impl *BiometricControllerImpl) generateSummaryRankingsForStepsDeltaMonth(sessCtx mongo.SessionContext, u *u_d.User, res *AggregatePointSummaryResponse, mu *sync.Mutex, wg *sync.WaitGroup) error {
	// Once this function has been completed (whether successfully or not) then
	// update the `WaitGroup` that this goroutine is finished.
	defer wg.Done()
	...
	rp, err := impl.RankPointStorer.GetByCompositeKeyForThisMonth(sessCtx, u.PrimaryHealthTrackingDevice.StepCountDeltaMetricID, rp_d.FunctionAverage)
	if err != nil {
...
func (impl *BiometricControllerImpl) generateSummaryRankingsForStepsDeltaYear(sessCtx mongo.SessionContext, u *u_d.User, res *AggregatePointSummaryResponse, mu *sync.Mutex, wg *sync.WaitGroup) error {
	// Once this function has been completed (whether successfully or not) then
	// update the `WaitGroup` that this goroutine is finished.
	defer wg.Done()

	rp, err := impl.RankPointStorer.GetByCompositeKeyForThisYear(sessCtx, u.PrimaryHealthTrackingDevice.StepCountDeltaMetricID, rp_d.FunctionAverage)
	if err != nil {

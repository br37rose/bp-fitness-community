...
func (impl *BiometricControllerImpl) generateSummaryDataForCaloriesBurned(sessCtx mongo.SessionContext, u *u_d.User, res *AggregatePointSummaryResponse, mu *sync.Mutex, wg *sync.WaitGroup) error {
	...
	// --- Today --- //
	go func() {
		if err := impl.generateSummaryDataForCaloriesBurnedToday(sessCtx, u, res, mu, wg); err != nil {
			//TODO
		}
	}()
	// --- Yesterday --- //
	go func() {
		if err := impl.generateSummaryDataForCaloriesBurnedYesterday(sessCtx, u, res, mu, wg); err != nil {
			//TODO
		}
	}()
	// --- This ISO Week --- //
	go func() {
		if err := impl.generateSummaryDataForCaloriesBurnedThisISOWeek(sessCtx, u, res, mu, wg); err != nil {
			//TODO
		}
	}()
	// --- Last ISO Week --- //
	go func() {
		if err := impl.generateSummaryDataForCaloriesBurnedLastISOWeek(sessCtx, u, res, mu, wg); err != nil {
			//TODO
		}
	}()
	// --- This Month --- //
	go func() {
		if err := impl.generateSummaryDataForCaloriesBurnedThisMonth(sessCtx, u, res, mu, wg); err != nil {
			//TODO
		}
	}()
	// --- Last Month --- //
	go func() {
		if err := impl.generateSummaryDataForCaloriesBurnedLastMonth(sessCtx, u, res, mu, wg); err != nil {
			//TODO
		}
	}()
	// --- This Year --- //
	go func() {
		if err := impl.generateSummaryDataForCaloriesBurnedThisYear(sessCtx, u, res, mu, wg); err != nil {
			//TODO
		}
	}()
	// --- Last Year --- //
	go func() {
		if err := impl.generateSummaryDataForCaloriesBurnedLastYear(sessCtx, u, res, mu, wg); err != nil {
			//TODO
		}
	}()
...
func (impl *BiometricControllerImpl) generateSummaryDataForCaloriesBurnedToday(sessCtx mongo.SessionContext, u *u_d.User, res *AggregatePointSummaryResponse, mu *sync.Mutex, wg *sync.WaitGroup) error {
	// Once this function has been completed (whether successfully or not) then
	// update the `WaitGroup` that this goroutine is finished.
	defer wg.Done()
	...
	thisDayFilter := &ap_s.AggregatePointPaginationListFilter{
		...
		MetricIDs: []primitive.ObjectID{u.PrimaryHealthTrackingDevice.CaloriesBurnedMetricID},
	...
	if thisDayList != nil {
		...
		// 	slog.String("metric_id", u.PrimaryHealthTrackingDeviceCaloriesBurnedMetricID.Hex()),
		// 	slog.Any("period", ap_s.PeriodHour),
		// 	slog.Any("sort_field", thisDayFilter.SortField),
		// 	slog.Any("sort_order", thisDayFilter.SortOrder),
		// 	slog.Time("start", thisDayStart),
		// 	slog.Time("end", thisDayEnd))
		res.CaloriesBurnedThisDayData = thisDayList.Results
	}

	return nil
}

func (impl *BiometricControllerImpl) generateSummaryDataForCaloriesBurnedYesterday(sessCtx mongo.SessionContext, u *u_d.User, res *AggregatePointSummaryResponse, mu *sync.Mutex, wg *sync.WaitGroup) error {
	// Once this function has been completed (whether successfully or not) then
	// update the `WaitGroup` that this goroutine is finished.
	defer wg.Done()
	...
	if lastDayList != nil {
		...
		// 	slog.Time("start", lastDayStart),
		// 	slog.Time("end", lastDayEnd))
		res.CaloriesBurnedLastDayData = lastDayList.Results
	}
	return nil
}

func (impl *BiometricControllerImpl) generateSummaryDataForCaloriesBurnedThisISOWeek(sessCtx mongo.SessionContext, u *u_d.User, res *AggregatePointSummaryResponse, mu *sync.Mutex, wg *sync.WaitGroup) error {
	// Once this function has been completed (whether successfully or not) then
	// update the `WaitGroup` that this goroutine is finished.
	defer wg.Done()
	...
	if thisISOWeekList != nil {
		...
		// 	slog.Time("start", thisISOWeekStart),
		// 	slog.Time("end", thisISOWeekEnd))
		res.CaloriesBurnedThisISOWeekData = thisISOWeekList.Results
	}

	return nil
}

func (impl *BiometricControllerImpl) generateSummaryDataForCaloriesBurnedLastISOWeek(sessCtx mongo.SessionContext, u *u_d.User, res *AggregatePointSummaryResponse, mu *sync.Mutex, wg *sync.WaitGroup) error {
	// Once this function has been completed (whether successfully or not) then
	// update the `WaitGroup` that this goroutine is finished.
	defer wg.Done()
	...
	if lastISOWeekWeekList != nil {
		...
		// 	slog.Time("start", lastISOWeekWeekStart),
		// 	slog.Time("end", lastISOWeekWeekEnd))
		res.CaloriesBurnedLastISOWeekData = lastISOWeekWeekList.Results
	}

	return nil
}

func (impl *BiometricControllerImpl) generateSummaryDataForCaloriesBurnedThisMonth(sessCtx mongo.SessionContext, u *u_d.User, res *AggregatePointSummaryResponse, mu *sync.Mutex, wg *sync.WaitGroup) error {
	// Once this function has been completed (whether successfully or not) then
	// update the `WaitGroup` that this goroutine is finished.
	defer wg.Done()
	...
	if thisMonthList != nil {
		...
		// 	slog.Time("start", thisMonthStart),
		// 	slog.Time("end", thisMonthEnd))
		res.CaloriesBurnedThisMonthData = thisMonthList.Results
	}

	return nil
}

func (impl *BiometricControllerImpl) generateSummaryDataForCaloriesBurnedLastMonth(sessCtx mongo.SessionContext, u *u_d.User, res *AggregatePointSummaryResponse, mu *sync.Mutex, wg *sync.WaitGroup) error {
	// Once this function has been completed (whether successfully or not) then
	// update the `WaitGroup` that this goroutine is finished.
	defer wg.Done()
	...
	if lastMonthList != nil {
		...
		// 	slog.Time("start", lastMonthStart),
		// 	slog.Time("end", lastMonthEnd))
		res.CaloriesBurnedLastMonthData = lastMonthList.Results
	}

	return nil
}

func (impl *BiometricControllerImpl) generateSummaryDataForCaloriesBurnedThisYear(sessCtx mongo.SessionContext, u *u_d.User, res *AggregatePointSummaryResponse, mu *sync.Mutex, wg *sync.WaitGroup) error {
	// Once this function has been completed (whether successfully or not) then
	// update the `WaitGroup` that this goroutine is finished.
	defer wg.Done()
	...
	if thisYearList != nil {
		...
		// 	slog.Time("start", thisYearStart),
		// 	slog.Time("end", thisYearEnd))
		res.CaloriesBurnedThisYearData = thisYearList.Results
	}

	return nil
}

func (impl *BiometricControllerImpl) generateSummaryDataForCaloriesBurnedLastYear(sessCtx mongo.SessionContext, u *u_d.User, res *AggregatePointSummaryResponse, mu *sync.Mutex, wg *sync.WaitGroup) error {
	// Once this function has been completed (whether successfully or not) then
	// update the `WaitGroup` that this goroutine is finished.
	defer wg.Done()
	...
	if lastYearList != nil {
		...
		// 	slog.Time("start", lastYearStart),
		// 	slog.Time("end", lastYearEnd))
		res.CaloriesBurnedLastYearData = lastYearList.Results
	}

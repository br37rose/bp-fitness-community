...
func (c *FitBitAppControllerImpl) fetchActivityDataForFitBit(
	...
	url := fmt.Sprintf(constants.FitBitGetActivityRateIntradayByDateURL,
	...
	if strings.Contains(responseStr, "Access token expired") {
		...
		return c.fetchActivityDataForFitBit(sessCtx, fba, startAt, resource, typeID)
	}

	// Convert into the data-structure set by FitBit inc.
	content := &fb_api.ActivitiesInterdayLog{}
	...
	formatted := fitbitdatum_s.FitBitFormattedDatum{
		ActivitiesInterdayLog: content,
	}

	c.Logger.Debug("unmarshalled datum",

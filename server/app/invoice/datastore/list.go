...
func (impl InvoiceStorerImpl) ListByFilter(ctx context.Context, f *InvoiceListFilter) (*InvoiceListResult, error) {
...
func (impl InvoiceStorerImpl) ListAsSelectOptionByFilter(ctx context.Context, f *InvoiceListFilter) ([]*InvoiceAsSelectOption, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 12*time.Second)
	defer cancel()
	...
	var results = []*InvoiceAsSelectOption{}
	if err = cursor.All(ctx, &results); err != nil {
		panic(err)
	}

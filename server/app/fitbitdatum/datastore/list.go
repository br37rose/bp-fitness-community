...
func (impl FitBitDatumStorerImpl) ListByFilter(ctx context.Context, f *FitBitDatumListFilter) (*FitBitDatumListResult, error) {
...
func (impl FitBitDatumStorerImpl) ListAsSelectOptionByFilter(ctx context.Context, f *FitBitDatumListFilter) ([]*FitBitDatumAsSelectOption, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 12*time.Second)
	defer cancel()
	...
	var results = []*FitBitDatumAsSelectOption{}
	if err = cursor.All(ctx, &results); err != nil {
		panic(err)
	}
